    ...
    <>
      {!isMobile && (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <DialogContent
            ref={contentRef}
            className="sm:max-w-sm bg-white border-[#7f5efd] shadow-xl"
          >
      ...
      {isMobile && (
        <BottomSheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <BottomSheetContent
            ref={contentRef}
            className="md:hidden max-h-[85vh] overflow-y-auto"
            onDismiss={onClose}
          >
            <BottomSheetHeader className="text-left pb-3">
              <BottomSheetTitle>{getTitle(currency)}</BottomSheetTitle>
              <BottomSheetDescription>
                Important information for {currency.toUpperCase()} payments
              </BottomSheetDescription>
            </BottomSheetHeader>
            ...
            <BottomSheetFooter className="pt-3">
              <Button
                onClick={onClose}
                className="bg-[#7f5efd] hover:bg-[#6b4fd8] text-white mobile-touch-button"
              >
                I Understand
              </Button>
            </BottomSheetFooter>
