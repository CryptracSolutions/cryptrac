...
export default function PaymentDetailsPage({ params }: PaymentDetailsPageProps) {
  ...
  const feeAmount = paymentLink.metadata?.fee_amount || 0;
  const feePercentage = paymentLink.metadata?.fee_percentage || 2.5;
  const totalAmount = paymentLink.metadata?.total_amount || paymentLink.amount;
